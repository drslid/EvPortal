...
La recette publique du 11 septembre a aussi validé le vrai QR sur GitHub Pages et le relais Cloudflare de production, avec deux navigateurs isolés : transfert d’une configuration synthétique chiffrée, aperçu avant application, persistance après rechargement et suppression confirmée par `DELETE 204`, puis `GET 410`. Aucune erreur JavaScript ni publication Telegra.ph n’a été détectée. Ce contrôle ne remplace pas l’essai sur véhicule physique.
