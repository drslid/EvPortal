...
(async () => {
    ...
    assert.ok(qrURL.startsWith('https://drslid.github.io/EvPortal/#receive='));
    ...
    await phone.goto(appURL + new URL(qrURL).hash, { waitUntil: 'networkidle' });
    ...
    await phone.goto(appURL + new URL(nextQR).hash);
    await phone.locator('#pairSendDialog').waitFor({ state: 'visible' });
