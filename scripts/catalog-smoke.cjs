...
(async () => {
    ...
    assert.equal(await page.evaluate(() => EVPreferences.createPreferences().read().market), undefined);
    assert.equal(await page.locator('#marketSelect, #catalogMarketToggle, #addShortcutButton, #addPageButton').count(), 0);
    ...
    assert.equal(await addOfficial.isEnabled(), true);
    assert.equal(await addOfficial.getAttribute('aria-label'), 'Supprimer ABRP');
    ...
    const personalBeforeCatalog = await saved(historic);
    await historic.locator('#catalogButton').click();
    await historic.locator('#catalogSearch').fill('ABRP');
    const historicalAction = historic.locator('[data-service-id="abrp"] .catalog-add');
    assert.equal(await historicalAction.getAttribute('aria-label'), 'Ajouter ABRP', 'Personal URLs never masquerade as installed catalogue identities');
    await historicalAction.click();
    assert.equal(entries(await saved(historic)).length, 3);
    assert.equal(await historicalAction.getAttribute('aria-label'), 'Supprimer ABRP');
    await historicalAction.click();
    assert.equal(entries(await saved(historic)).length, 2);
    for (const shortcutID of ['custom-a', 'custom-b']) assert.deepEqual(entry(await saved(historic), shortcutID).shortcut, entry(personalBeforeCatalog, shortcutID).shortcut);
    assert.equal(await historicalAction.getAttribute('aria-label'), 'Ajouter ABRP');
    assert.equal(await historicalAction.evaluate(button => document.activeElement === button), true);
    await close(historic, 'catalogDialog');
    await historic.reload({ waitUntil: 'networkidle' });
    assert.equal(entries(await saved(historic)).length, 2);
    console.log('✓ Catalogue removal uses service identity, preserves personal URLs and their counters, persists, and keeps Add available at the same focus position');

    const grouped = await freshPage();
    const defaultCatalogue = await saved(grouped);
    const originalABRP = service(defaultCatalogue, 'abrp').shortcut;
    await grouped.locator('#catalogButton').click();
    assert.equal(await grouped.locator('.catalog-group').count(), await grouped.evaluate(() => EV_CATALOG.categories.length));
    assert.equal(await grouped.locator('.catalog-group[open]').count(), 1);
    assert.equal(await grouped.locator('.catalog-group[open]').getAttribute('data-category-id'), 'cinema');
    assert.equal(await grouped.locator('[data-service-id="crave"]').count(), 1, 'The global catalogue includes services from all countries');
    assert.equal(await grouped.locator('[data-service-id="netflix"] .shortcut-icon img').count(), 1, 'The service row uses its local favicon');
    for (const serviceID of ['disney-plus', 'paramount-plus']) assert.equal(await grouped.locator('[data-service-id="' + serviceID + '"] strong').getAttribute('dir'), 'auto', 'Brand punctuation is isolated from the surrounding RTL interface');
    await grouped.locator('.catalog-group[data-category-id="music"] > summary').click();
    await grouped.locator('.catalog-group[data-category-id="cinema"] > summary').click();
    await grouped.locator('#catalogSearch').fill('ABRP');
    assert.equal(await grouped.locator('.catalog-group[open]').getAttribute('data-category-id'), 'charging');
    const removeShared = grouped.locator('[data-service-id="abrp"] .catalog-add');
    await removeShared.click();
    assert.equal(service(await saved(grouped), 'abrp'), undefined);
    assert.equal(entry(await saved(grouped), originalABRP.id), undefined);
    assert.equal(await removeShared.getAttribute('aria-label'), 'Ajouter ABRP');
    assert.equal(await removeShared.evaluate(button => document.activeElement === button), true);
    await grouped.locator('#catalogSearch').fill('');
    assert.equal(await grouped.locator('.catalog-group[open]').count(), 1);
    assert.equal(await grouped.locator('.catalog-group[open]').getAttribute('data-category-id'), 'music', 'Clearing search restores the user-selected open groups');
    await close(grouped, 'catalogDialog');
    await category(grouped, 'charging');
    assert.equal(await card(grouped, originalABRP.id).count(), 0);
    await category(grouped, 'navigation');
    assert.equal(await card(grouped, originalABRP.id).count(), 0);
    await category(grouped, 'all');
    assert.equal(await card(grouped, originalABRP.id).count(), 0);
    await grouped.locator('#catalogButton').click();
    assert.equal(await grouped.locator('.catalog-group[open]').getAttribute('data-category-id'), 'music', 'Reopening the catalogue preserves its open groups');
    await grouped.locator('#catalogSearch').fill('ABRP');
    await removeShared.click();
    assert.deepEqual(service(await saved(grouped), 'abrp').shortcut.categoryIds, ['charging', 'navigation']);
    assert.equal(await removeShared.getAttribute('aria-label'), 'Supprimer ABRP');
    await grouped.locator('#catalogSearch').fill('');
    assert.equal(await grouped.locator('.catalog-group[open]').getAttribute('data-category-id'), 'music');
    console.log('✓ Global catalogue groups preserve expansion across searching and actions; shared services remove everywhere and can be re-added');

    const creation = await freshPage();
    await category(creation, 'all');
    await creation.locator('#catalogButton').click();
    await creation.locator('#catalogCustomButton').click();
