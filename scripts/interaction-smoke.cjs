...
(async () => {
    ...
    const deletion = await pageFor();
    const dialogs = [];
    deletion.on('dialog', async dialog => { dialogs.push({ type: dialog.type(), message: dialog.message() }); await dialog.dismiss(); });
    await deletion.locator('#editModeToggle').click();
    await deletion.locator('[data-category="favorites"]').click();
    await deletion.getByRole('button', { name: 'Supprimer Alpha', exact: true }).click();
    assert.deepEqual(dialogs, []);
    assert.deepEqual(await ids(deletion), ['c']);
    assert.deepEqual((await saved(deletion)).categories.flatMap(category => category.shortcuts).map(shortcut => shortcut.id), ['b', 'c', 'd']);
    assert.match(await deletion.locator('#statusMessage').textContent(), /Raccourci supprimé/);
    await deletion.locator('[data-category="second"]').click();
    await deletion.getByRole('button', { name: 'Supprimer la catégorie Seconde', exact: true }).click();
    assert.deepEqual(dialogs, []);
    assert.deepEqual((await saved(deletion)).categories.map(category => category.id), ['first']);
    assert.equal((await saved(deletion)).activeCategory, 'all');
    assert.equal(await deletion.evaluate(() => document.activeElement.dataset.category), 'all');
    assert.deepEqual(await ids(deletion), ['b']);
    assert.match(await deletion.locator('#statusMessage').textContent(), /Catégorie supprimée/);
    await deletion.locator('[data-category="favorites"]').click();
    assert.deepEqual(await ids(deletion), []);
    await deletion.reload({ waitUntil: 'networkidle' });
    assert.deepEqual(await ids(deletion), []);
    await deletion.locator('#editModeToggle').click();
    await deletion.getByRole('button', { name: 'Supprimer la catégorie Première', exact: true }).click();
    assert.deepEqual(dialogs, []);
    assert.equal((await saved(deletion)).categories.length, 0);
    ...
    assert.match(dialogs[0].message, /Rétablir le catalogue/);
    assert.equal((await saved(deletion)).categories.length, 0);
    console.log('✓ Shortcut and category deletions persist without dialogs; empty Tous/Favoris stay valid and full reset still requires confirmation');
